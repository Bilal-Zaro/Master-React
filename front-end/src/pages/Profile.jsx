...
export default function Profile() {
  ...
  useEffect(() => {
    // Mock data for tasks
    const mockTasks = [
      { id: 1, status: "doing", priority: "high", deleted: false },
      { id: 2, status: "done", priority: "medium", deleted: false },
      { id: 3, status: "toDo", priority: "low", deleted: false },
      // Add more tasks as needed
    ];

    setData(mockTasks);

    const doing = mockTasks.filter(
      (task) => task.status === "doing" && task.deleted !== true
    );
    const done = mockTasks.filter(
      (task) => task.status === "done" && task.deleted !== true
    );
    const toDo = mockTasks.filter(
      (task) => task.status === "toDo" && task.deleted !== true
    );

    const high = mockTasks.filter(
      (task) => task.priority === "high" && task.deleted !== true
    );
    const medium = mockTasks.filter(
      (task) => task.priority === "medium" && task.deleted !== true
    );
    const low = mockTasks.filter(
  ...
  useEffect(() => {
    ...
    const mockAssignedTasks = [
      { id: 1, assignedTo: "123", task: "Task 1" },
      { id: 2, assignedTo: "123", task: "Task 2" },
    ];
